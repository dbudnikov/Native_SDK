...
Vector Vector::vecMul(Vector lhs, Vector mhs, Vector rhs, uint32_t loop_size)
{
	...
	for (size_t x = 0; x < newWidth; x++)
	{
		...
		{
			int index = k%10;
			sum += (lhs(x) * lhs(x) * lhs(x) * const_array[index] +
			        lhs(x) * lhs(x) * rhs(x) * const_array[index] +
					mhs(x) * mhs(x) * rhs(x) * const_array[index] +
					lhs(x) * mhs(x) * mhs(x) * const_array[index] +
					lhs(x) * rhs(x) * rhs(x) * const_array[index] +
					lhs(x) * mhs(x) * rhs(x) * const_array[index] +
					rhs(x) * rhs(x) * rhs(x) * const_array[index] +
					mhs(x) * rhs(x) * rhs(x) * const_array[index] +
					lhs(x) * lhs(x) * mhs(x) * const_array[index] +
					mhs(x) * mhs(x) * mhs(x) * const_array[index]);
		}
		m[x] = sum;
	}
